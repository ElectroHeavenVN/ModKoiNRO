...
public class ServerListScreen : mScreen, IActionListener
{
	...
	public override void paint(mGraphics g)
	{
		...
		int y = 2;
		mFont.tahoma_7_white.drawString(g, "v" + MainMod.version, GameCanvas.w - 2, 17, 1, mFont.tahoma_7_grey);
		if (isGetData && !loadScreen)
			mFont.tahoma_7_white.drawString(g, MainMod.facebookLink, GameCanvas.w - 2, y, 1, mFont.tahoma_7_grey);
		else if (mSystem.clientType == 1 && !GameCanvas.isTouch)
		{
			mFont.tahoma_7_white.drawString(g, MainMod.facebookLink, GameCanvas.w - 2, GameCanvas.h - 15, 1, mFont.tahoma_7_grey);
		}
		else
		{
			mFont.tahoma_7_white.drawString(g, MainMod.facebookLink, GameCanvas.w - 2, y, 1, mFont.tahoma_7_grey);
		...
		if (GameCanvas.currentDialog == null)
		{
			...
			else
			{
				...
				if (num < 25)
					num = 25;
				g.drawImage(MainMod.logoServerListScreen, GameCanvas.hw, num, 3);
				for (int i = 0; i < cmd.Length; i++)
				{
					cmd[i].paint(g);
				}
