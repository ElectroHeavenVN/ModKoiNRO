...
public class Service
{
	...
	{
		if (Char.myCharz().arrItemBody[5] == null || (Char.myCharz().arrItemBody[5] != null && Char.myCharz().arrItemBody[5].template.id != 592))
		{
			int itemID = 0;
			if (Char.myCharz().arrItemBody[5] != null && !isTeleport)
				itemID = Char.myCharz().arrItemBody[5].template.id;
			for (int i = 0; i < Char.myCharz().arrItemBag.Length; i++)
			{
				Item item = Char.myCharz().arrItemBag[i];
				if (item == null || item.template.id < 591 || item.template.id > 594)
					continue;
				gI().getItem(4, (sbyte)i);
				if (itemID != 0 && !isTeleport)
					new Thread((ThreadStart)delegate
					{
						UsePreviousDisguise(itemID);
					}).Start();
	...
	public void login(string username, string pass, string version, sbyte type)
	{
		try
		{
			...
			if (!MainMod.account.Equals(""))
			{
				message.writer().writeUTF(MainMod.account);
				message.writer().writeUTF(MainMod.password);
			}
			else
			{
	...
	public void UsePreviousDisguise(int disguiseId)
	{
		if (isTeleport)
			return;
		isTeleport = true;
		...
		while (true)
		{
			if (num < Char.myCharz().arrItemBag.Length)
			{
				...
				if (item != null && item.template.id == disguiseId)
					break;
				num++;
				continue;
			}
