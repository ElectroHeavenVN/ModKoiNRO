...
public class Panel : IActionListener, IChatable
{
	...
	public int getXMap()
	{
		...
		while (true)
		{
			if (num < mapId[TileMap.planetID].Length)
			{
				if (TileMap.mapID == mapId[TileMap.planetID][num])
					break;
				num++;
				continue;
			}
		...
		return mapX[TileMap.planetID][num];
	}
	...
	public int getYMap()
	{
		...
		return mapY[TileMap.planetID][num];
	}
	...
	public int getXMapTask()
	{
		...
		while (true)
		{
			if (num < mapId[TileMap.planetID].Length)
			{
				if (GameScr.mapTasks[Char.myCharz().taskMaint.index] == mapId[TileMap.planetID][num])
					break;
				num++;
				continue;
			}
			...
			if (num < currentListLength)
			{
				...
				if (num3 - cmy <= yScroll + hScroll && num3 - cmy >= yScroll - ITEM_HEIGHT)
				{
					if (num == currentListLength - 1)
					{
						...
						{
							g.setColor(15196114);
							g.fillRect(num6, num3, wScroll, num5);
							if (!isLock)
							{
								if (!isFriendLock)
									mFont.tahoma_7_grey.drawString(g, mResources.opponent + mResources.not_lock_trade, xScroll + wScroll / 2, num3 + num5 / 2 - 4, mFont.CENTER);
								else
									mFont.tahoma_7_grey.drawString(g, mResources.opponent + mResources.locked_trade, xScroll + wScroll / 2, num3 + num5 / 2 - 4, mFont.CENTER);
							}
							else if (isFriendLock)
							{
								g.setColor(15196114);
								g.fillRect(num6, num3, wScroll, num5);
								g.drawImage((num != selected) ? GameScr.imgLbtn2 : GameScr.imgLbtnFocus2, xScroll + wScroll - 5, num3 + 2, StaticObj.TOP_RIGHT);
								((num != selected) ? mFont.tahoma_7b_dark : mFont.tahoma_7b_green2).drawString(g, mResources.done, xScroll + wScroll - 22, num3 + 7, 2);
								mFont.tahoma_7_grey.drawString(g, mResources.opponent + mResources.locked_trade, xScroll + 5, num3 + num5 / 2 - 4, mFont.LEFT);
							}
							else
							{
								mFont.tahoma_7_grey.drawString(g, mResources.opponent + mResources.not_lock_trade, xScroll + wScroll / 2, num3 + num5 / 2 - 4, mFont.CENTER);
					...
					else if (num == currentListLength - 2)
					{
						if (isMe)
						{
							...
							if (!isAccept)
							{
								...
								{
									g.drawImage((num != selected) ? GameScr.imgLbtn2 : GameScr.imgLbtnFocus2, xScroll + wScroll - 5, num3 + 2, StaticObj.TOP_RIGHT);
									((num != selected) ? mFont.tahoma_7b_dark : mFont.tahoma_7b_green2).drawString(g, mResources.mlock, xScroll + wScroll - 22, num3 + 7, 2);
									mFont.tahoma_7_grey.drawString(g, mResources.you + mResources.not_lock_trade, xScroll + 5, num3 + num5 / 2 - 4, mFont.LEFT);
								}
								else
								{
									g.drawImage((num != selected) ? GameScr.imgLbtn2 : GameScr.imgLbtnFocus2, xScroll + wScroll - 5, num3 + 2, StaticObj.TOP_RIGHT);
									((num != selected) ? mFont.tahoma_7b_dark : mFont.tahoma_7b_green2).drawString(g, mResources.CANCEL, xScroll + wScroll - 22, num3 + 7, 2);
									mFont.tahoma_7_grey.drawString(g, mResources.you + mResources.locked_trade, xScroll + 5, num3 + num5 / 2 - 4, mFont.LEFT);
								}
							}
						}
						else if (!isFriendLock)
						{
							mFont.tahoma_7b_dark.drawString(g, mResources.not_lock_trade_upper, xScroll + wScroll / 2, num3 + num5 / 2 - 4, mFont.CENTER);
						}
						else
						{
							mFont.tahoma_7b_dark.drawString(g, mResources.locked_trade_upper, xScroll + wScroll / 2, num3 + num5 / 2 - 4, mFont.CENTER);
						}
					}
					else if (num == currentListLength - 3)
					{
						if (isLock)
							...
							g.setColor((num != selected) ? 15196114 : 16383818);
						g.fillRect(num2, num3, num4, num5);
						if (isLock)
							g.setColor(13748667);
						else
							g.setColor((num != selected) ? 9993045 : 7300181);
						g.fillRect(num6, num7, num8, num9);
						g.drawImage(imgXu, num6 + num8 / 2, num7 + num9 / 2, 3);
						mFont.tahoma_7_green2.drawString(g, NinjaUtil.getMoneys((!isMe) ? friendMoneyGD : moneyGD) + " " + mResources.XU, num2 + 5, num3 + 11, 0);
						mFont.tahoma_7_green.drawString(g, mResources.money_trade, num2 + 5, num3, 0);
					}
					else
					{
						if (myVector.size() == 0)
						...
						if (isLock)
							...
							g.setColor((num != selected) ? 9993045 : 9541120);
						g.fillRect(num6, num7, num8, num9);
						Item item = (Item)myVector.elementAt(num);
						if (item != null)
						{
							string text = string.Empty;
							if (item.itemOption != null)
							...
							if (item.itemOption != null)
							{
								...
								{
									for (int j = 1; j < item.itemOption.Length; j++)
									{
										if (item.itemOption[j] != null && item.itemOption[j].optionTemplate.id != 102 && item.itemOption[j].optionTemplate.id != 107)
											text2 = text2 + "," + item.itemOption[j].getOptionString();
									}
								}
	...
	private void paintZone(mGraphics g)
	{
		...
		for (int i = 0; i < pts.Length; i++)
		{
			...
			if (zones[i] != -1)
			{
				...
				if (zoneID == -1 || zoneID != i)
					mFont.tahoma_7_green2.drawString(g, GameScr.gI().numPlayer[i] + "/" + GameScr.gI().maxPlayer[i] + ((GameScr.gI().numPlayer[i] < GameScr.gI().maxPlayer[i] || TileMap.zoneID == i) ? "" : " [Chọn để auto vào khu]"), num + 5, num2 + 6, 0);
				else
				{
					mFont.tahoma_7b_red.drawString(g, GameScr.gI().numPlayer[i] + "/" + GameScr.gI().maxPlayer[i] + " [Đang auto vào khu]", num + 5, num2 + 6, 0);
					if (GameScr.gI().numPlayer[i] < GameScr.gI().maxPlayer[i])
					{
						zoneID = -1;
	...
	private void paintInventory(mGraphics g)
	{
		...
		for (int i = 0; i < arrItemBody.Length + arrItemBag.Length; i++)
		{
			...
			if (AutoItem.set1.Contains(item.getFullName()))
				text = ((!AutoItem.set2.Contains(item.getFullName())) ? (text + " - Set 1") : (text + " - Set 1,2"));
			else if (AutoItem.set2.Contains(item.getFullName()))
			{
				text += " - Set 2";
			}
	...
	private void paintBottomMoneyInfo(mGraphics g)
	{
		if (type != 13 || (currentTabIndex != 2 && !Equals(GameCanvas.panel2)))
		{
			...
			g.drawImage(imgXu, X + 10, H - 7, 3);
			g.drawImage(imgLuong, X + 80, H - 8, 3);
			mFont.tahoma_7_yellow.drawString(g, NinjaUtil.getMoneys(Char.myCharz().xu) + string.Empty, X + 20, H - 13, mFont.LEFT, mFont.tahoma_7_grey);
			mFont.tahoma_7_yellow.drawString(g, NinjaUtil.getMoneys(Char.myCharz().luong) + string.Empty, X + 90, H - 13, mFont.LEFT, mFont.tahoma_7_grey);
	...
	private void paintClanInfo(mGraphics g)
	{
		...
		else if (!isViewMember)
		{
			...
			if (clan != null)
			{
				...
				mFont.tahoma_7_yellow.drawString(g, mResources.clan_point + ": " + clan.clanPoint + "  -  " + mResources.level + ": " + clan.level, 60, 27, mFont.LEFT, mFont.tahoma_7_grey);
	...
	{
		mFont.tahoma_7_yellow.drawString(g, mResources.power + ": " + NinjaUtil.getMoneys(Char.myPetz().cPower), X + 52, 4, mFont.LEFT, mFont.tahoma_7_grey);
		if (Char.myPetz().cPower > 0L)
			mFont.tahoma_7_yellow.drawString(g, (!Char.myPetz().me) ? Char.myPetz().currStrLevel : Char.myPetz().getStrLevel(), X + 52, 16, mFont.LEFT, mFont.tahoma_7_grey);
		if (Char.myPetz().cDamFull > 0)
			mFont.tahoma_7_yellow.drawString(g, mResources.hit_point + ": " + NinjaUtil.getMoneys(Char.myPetz().cDamFull), X + 52, 27, mFont.LEFT, mFont.tahoma_7_grey);
		if (Char.myPetz().cMaxStamina > 0)
			mFont.tahoma_7_yellow.drawString(g, string.Concat(new object[5]
			{
				mResources.vitality,
				": ",
				NinjaUtil.getMoneys(Char.myPetz().cStamina),
				" / ",
				NinjaUtil.getMoneys(Char.myPetz().cMaxStamina)
	...
	private void paintCharInfo(mGraphics g, Char c)
	{
		...
		if (c.cMaxStamina > 0)
		{
			...
			g.setClip(w: c.cStamina * mGraphics.getImageWidth(GameScr.imgMP) / c.cMaxStamina, x: 95, y: X + 19, h: 20);
	...
	private void paintItemBodyBagInfo(mGraphics g)
	{
		...
		mFont.tahoma_7_yellow.drawString(g, mResources.hit_point + ": " + NinjaUtil.getMoneys(Char.myCharz().cDamFull), X + 52, 26, mFont.LEFT, mFont.tahoma_7_grey);
		mFont.tahoma_7_yellow.drawString(g, string.Concat(new object[8]
		{
			mResources.armor,
			": ",
			...
			mResources.critical,
			": ",
			NinjaUtil.getMoneys(Char.myCharz().cCriticalFull),
			"%"
		}), X + 52, 38, mFont.LEFT, mFont.tahoma_7_grey);
	}
	...
	{
		mFont.tahoma_7b_white.drawString(g, string.Concat(new object[4]
		{
			"HP: ",
		...
		mFont.tahoma_7_yellow.drawString(g, string.Concat(new object[7]
		{
			mResources.critical,
			": ",
			NinjaUtil.getMoneys(Char.myPetz().cCriticalFull),
			"   ",
			mResources.armor,
			": ",
			NinjaUtil.getMoneys(Char.myPetz().cDefull)
		}), X + 52, 27, mFont.LEFT, mFont.tahoma_7_grey);
		mFont.tahoma_7_yellow.drawString(g, mResources.status + ": " + strStatus[Char.myPetz().petStatus], X + 52, 38, mFont.LEFT, mFont.tahoma_7_grey);
	}
	...
	public void paintTask(mGraphics g)
	{
		...
		if (indexMenu == 0)
		{
			...
			else
			{
				...
				{
					if (Char.myCharz().taskMaint == null && Char.myCharz().ctaskId == 9 && Char.myCharz().nClass.classId == 0)
	...
	private void doFireShop()
	{
			...
			if (currItem != null)
			{
				...
				else if (currItem.buyCoin == 0 && currItem.buyGold == 0)
				{
					...
					else
					{
						...
						myVector.addElement(new Command("Mua Nhiều", AutoItem.getInstance(), 3, new AutoItem.ItemAuto(currItem.template.id, -1, true, false)));
					}
				}
				else if (typeShop != 2)
				{
					if (currItem.buyCoin > 0)
					{
						...
						myVector.addElement(new Command("Mua Nhiều\n" + Res.formatNumber2(currItem.buyCoin) + " Vàng/1", AutoItem.getInstance(), 3, new AutoItem.ItemAuto(currItem.template.id, -1, true, false)));
					}
					if (currItem.buyGold > 0)
					{
						myVector.addElement(new Command(mResources.buy_with + "\n" + Res.formatNumber2(currItem.buyGold) + "\n" + mResources.LUONG, this, 3001, currItem));
						myVector.addElement(new Command("Mua Nhiều\n" + Res.formatNumber2(currItem.buyGold) + " Ngọc/1", AutoItem.getInstance(), 3, new AutoItem.ItemAuto(currItem.template.id, -1, false, false)));
					}
				}
				else
				{
		...
		else if (typeShop == 0)
		{
			...
			{
				int num = selected;
				IntPtr intPtr = (nint)arrItemBody.LongLength;
				Item item = Char.myCharz().arrItemBag[(sbyte)(num - (int)(nint)intPtr)];
				if (item != null)
					currItem = item;
			}
			else
	...
	private void doFireInventory()
	{
			...
			if (item != null)
			{
				...
				else
				{
					if (item.isTypeBody())
					{
						...
						if (Equals(GameCanvas.panel))
						{
							num2 = AutoItem.set1.Contains(item.getFullName());
							flag = AutoItem.set2.Contains(item.getFullName());
							if (!num2 && !flag)
							{
		...
		else
		{
			...
			if (item2 != null)
			{
				...
				if (Equals(GameCanvas.panel))
				{
					num3 = AutoItem.set1.Contains(item2.getFullName());
					flag2 = AutoItem.set2.Contains(item2.getFullName());
					if (!num3 && !flag2)
					{
						myVector.addElement(new Command("Thêm Vào\nSet 1", AutoItem.getInstance(), 4, currItem));
						myVector.addElement(new Command("Thêm Vào\nSet 2", AutoItem.getInstance(), 5, currItem));
		...
		if (currItem != null)
		{
			...
			if (GameCanvas.panel.type != 12 && GameCanvas.panel.type != 13)
			{
				...
				if (position == 1)
					myVector.addElement(new Command(mResources.SALE, this, 3002, currItem));
				if (currItem.template.id == 457 && currItem.quantity > 1)
					myVector.addElement(new Command("Bán Nhiều", AutoItem.getInstance(), 3, new AutoItem.ItemAuto(currItem.template.id, (short)currItem.indexUI, false, true)));
			}
			GameCanvas.menu.startAt(myVector, X, (selected + 1) * ITEM_HEIGHT - cmy + yScroll);
			addItemDetail(currItem);
		}
	...
	private void doFireSkill()
	{
		...
		{
			int num = selected;
			SkillTemplate skillTemplate = Char.myCharz().nClass.skillTemplates[num - 6];
			Skill skill = Char.myCharz().getSkill(skillTemplate);
			Skill skill2 = null;
			MyVector myVector = new MyVector();
			if (skill != null)
		...
		if (selected == 2)
		{
			...
			if (cTiemNang >= 100 * (2 * cDamGoc + 99) / 2 * Char.myCharz().expForOneAdd)
			{
				...
				myVector7.addElement(new Command(mResources.increase_upper + "\n" + Char.myCharz().damFrom1000TiemNang + "\n" + mResources.hit_point + "\n-" + cDamGoc * 100, this, 9000, null));
				myVector7.addElement(new Command(mResources.increase_upper + "\n" + 10 * Char.myCharz().damFrom1000TiemNang + "\n" + mResources.hit_point + "\n-" + 10 * (2 * cDamGoc + 9) / 2 * Char.myCharz().expForOneAdd, this, 9006, null));
	...
	private void doFireBox()
	{
		...
		if (currentTabIndex == 1 || Equals(GameCanvas.panel2))
		{
			...
			if (selected >= arrItemBody.Length)
			{
				int num = selected;
				IntPtr intPtr = (nint)arrItemBody.LongLength;
				Item item2 = Char.myCharz().arrItemBag[(sbyte)(num - (int)(nint)intPtr)];
				if (item2 != null)
				{
	...
	public void perform(int idAction, object p)
	{
		if (idAction == 9999)
		{
			TopInfo obj = (TopInfo)p;
			Service.gI().sendThachDau(obj.pId);
		}
		if (idAction == 170391)
		{
			Rms.clearAll();
		...
		if (idAction == 8003)
		{
			InfoItem obj2 = (InfoItem)p;
			Service.gI().friend(1, obj2.charInfo.charID);
			if (type != 8)
				;
		}
		if (idAction == 8002)
		{
			InfoItem obj3 = (InfoItem)p;
			Service.gI().friend(2, obj3.charInfo.charID);
		}
		if (idAction == 8004)
		{
			InfoItem obj4 = (InfoItem)p;
			Service.gI().gotoPlayer(obj4.charInfo.charID);
		}
		if (idAction == 8001)
		{
			...
			chatTField.tfChat.name = mResources.chat_with + " " + infoItem.charInfo.cName;
			chatTField.to = string.Empty;
			chatTField.isShow = true;
			chatTField.tfChat.isFocus = true;
			chatTField.tfChat.setIputType(TField.INPUT_TYPE_ANY);
		...
		if (idAction == 1001)
		{
			...
			int num = selected;
			IntPtr intPtr = (nint)arrItemBody.LongLength;
			Service.gI().getItem(BAG_BOX, (sbyte)(num - (int)(nint)intPtr));
		}
		if (idAction == 1003)
			hide();
		if (idAction == 1002)
		...
		if (idAction == 2000)
		{
			...
			int num2 = selected;
			IntPtr intPtr2 = (nint)arrItemBody2.LongLength;
			Service.gI().getItem(BAG_BODY, (sbyte)(num2 - (int)(nint)intPtr2));
		}
		if (idAction == 2001)
		{
			Res.outz("use item");
		...
		if (idAction == 2005)
		{
			...
			int num3 = selected;
			IntPtr intPtr3 = (nint)arrItemBody3.LongLength;
			Service.gI().getItem(BAG_PET, (sbyte)(num3 - (int)(nint)intPtr3));
		}
		if (idAction == 2006)
		{
			_ = Char.myPetz().arrItemBody;
			int num4 = selected;
			Service.gI().getItem(PET_BAG, (sbyte)num4);
		}
		if (idAction == 2002)
		...
		if (idAction == 4006)
		{
			ClanMessage obj8 = (ClanMessage)p;
			Service.gI().clanDonate(obj8.id);
		}
		if (idAction == 5001)
		{
			Member obj9 = (Member)p;
			Service.gI().clanRemote(obj9.ID, 0);
		}
		if (idAction == 5002)
		{
			Member obj10 = (Member)p;
			Service.gI().clanRemote(obj10.ID, 1);
		}
		if (idAction == 5003)
		{
			Member obj11 = (Member)p;
			Service.gI().clanRemote(obj11.ID, 2);
		}
		if (idAction == 5004)
		{
			Member obj12 = (Member)p;
			Service.gI().clanRemote(obj12.ID, -1);
		}
		if (idAction == 9000)
		{
		...
		if (idAction == 10000)
		{
			InfoItem obj13 = (InfoItem)p;
			Service.gI().enemy(1, obj13.charInfo.charID);
			GameCanvas.panel.hideNow();
		}
		if (idAction == 10001)
		{
			InfoItem obj14 = (InfoItem)p;
			Service.gI().enemy(2, obj14.charInfo.charID);
			InfoDlg.showWait();
		}
		if (idAction == 10021)
			;
	...
	private void doNotiRuby(int type)
	{
		...
		GameCanvas.startYesNoDlg(cmdYes: new Command(mResources.YES, this, (type != 0) ? 11001 : 11000, null), cmdNo: new Command(mResources.NO, this, 11002, null), info: mResources.notiRuby);
	}
