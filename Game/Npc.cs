			...
			else if (template.npcTemplateId != 4)
			{
						...
						if (TileMap.mapID != 51)
						{
							...
							if (Char.myCharz().npcFocus != null && Char.myCharz().npcFocus.Equals(this))
							{
								if (ChatPopup.currChatPopup == null)
									g.drawRegion(Mob.imgHP, 0, 0, 9, 6, 0, cx, cy - ch - (num - 8), mGraphics.BOTTOM | mGraphics.HCENTER);
							}
							else
							{
								num = 8;
				...
				else if (duahau != null)
				{
					...
					if (SmallImage.imgNew[duahau[duaHauIndex]] != null && SmallImage.imgNew[duahau[duaHauIndex]].img != null)
						num2 = mGraphics.getImageHeight(SmallImage.imgNew[duahau[duaHauIndex]].img);
					SmallImage.drawSmallImage(g, duahau[duaHauIndex], cx + Res.random(-1, 1), cy, 0, mGraphics.BOTTOM | mGraphics.HCENTER);
					if (Char.myCharz().npcFocus != null && Char.myCharz().npcFocus.Equals(this))
					{
						if (ChatPopup.currChatPopup == null)
							g.drawRegion(Mob.imgHP, 0, 0, 9, 6, 0, cx, cy - ch - 9 + 16 - num2, mGraphics.BOTTOM | mGraphics.HCENTER);
						mFont.tahoma_7b_white.drawString(g, NinjaUtil.getTime(seconds), cx, cy - ch - 16 - mFont.tahoma_7.getHeight() - 20 - num2 + 16, mFont.CENTER, mFont.tahoma_7b_dark);
					}
					else
						mFont.tahoma_7b_white.drawString(g, NinjaUtil.getTime(seconds), cx, cy - ch - 8 - mFont.tahoma_7.getHeight() - 20 - num2 + 16, mFont.CENTER, mFont.tahoma_7b_dark);
				}
			}
		...
		else
		{
				...
				if (TileMap.mapID != 51)
				{
					...
					else
					{
						...
						if (template.npcTemplateId == 47)
							num = 40;
						if (TileMap.mapID != 113)
							mFont.tahoma_7_yellow.drawString(g, template.name, cx, cy - ch - num - mFont.tahoma_7.getHeight(), mFont.CENTER, mFont.tahoma_7_grey);
					}
				}
				dyEff = 65;
			}
			else if (duahau != null)
			{
				...
				if (Char.myCharz().npcFocus != null && Char.myCharz().npcFocus.Equals(this))
					...
					mFont.tahoma_7_yellow.drawString(g, template.name, cx, cy - ch - 8 - mFont.tahoma_7.getHeight() - num2 + 16, mFont.CENTER, mFont.tahoma_7_grey);
			}
		}
	}
