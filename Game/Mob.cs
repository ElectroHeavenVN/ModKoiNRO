...
public class Mob : IMapObject
{
	...
	public static MyVector lastMob;

	public static MyVector newMob;
	...
	public static bool isExistNewMob(string id)
	{
		...
		while (true)
		{
			if (num < newMob.size())
			{
				if (((string)newMob.elementAt(num)).Equals(id))
					break;
				num++;
				continue;
			}
	...
	static Mob()
	{
		lastMob = new MyVector();
		newMob = new MyVector();
		imgHP = GameCanvas.loadImage("/mainImage/myTexture2dmobHP.png");
	}
}
