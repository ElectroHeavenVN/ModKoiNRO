public class BackgroudEffect
{
	...
	public BackgroudEffect(int typeS)
	{
		...
		case 10:
		{
			...
			{
				x[j] = Res.abs(Res.random(0, GameCanvas.w)) + GameScr.cmx;
				num++;
				if (num > this.num / 2)
				{
					y[j] = Res.abs(Res.random(20, 60));
					wP[j] = 10;
				}
				else
				{
					y[j] = Res.abs(Res.random(0, 20));
					wP[j] = 7;
				}
				vx[j] = wP[j] / 2 - 2;
			}
		...
		case 11:
		{
			...
			for (int m = 0; m < sum; m++)
			{
				x[m] = Res.random(-10, TileMap.pxw + 10);
				y[m] = Res.random(0, TileMap.pxh);
				frame[m] = Res.random(0, 1);
				t[m] = Res.random(0, 1);
				vx[m] = Res.random(-3, 3);
				vy[m] = Res.random(1, 4);
				if (typeEff == 11)
				{
					frame[m] = Res.random(0, 2);
					vx[m] = Res.abs(Res.random(1, 3));
					vy[m] = Res.abs(Res.random(1, 3));
				}
			}
			break;
		}
	...
	public static bool isHaveRain()
	{
		...
		while (true)
		{
			if (num < vBgEffect.size())
			{
				BackgroudEffect backgroudEffect = (BackgroudEffect)vBgEffect.elementAt(num);
	...
	public void update()
	{
		...
		case 11:
		{
			for (int k = 0; k < sum; k++)
			{
				if (k % 3 != 0 && TileMap.tileTypeAt(x[k], y[k] + ((TileMap.tileID == 15) ? 10 : 0), 2))
					activeEff[k] = true;
				if (k % 3 == 0 && y[k] > TileMap.pxh)
				{
					x[k] = Res.random(-10, TileMap.pxw + 50);
					y[k] = Res.random(-50, 0);
				}
				if (!activeEff[k])
				{
					for (int l = 0; l < Teleport.vTeleport.size(); l++)
					{
						Teleport teleport = (Teleport)Teleport.vTeleport.elementAt(l);
						if (teleport != null && teleport.paintFire && x[k] < teleport.x + 80 && x[k] > teleport.x - 80 && y[k] < teleport.y + 80 && y[k] > teleport.y - 80)
							x[k] += ((x[k] >= teleport.x) ? 10 : (-10));
					}
					y[k] += vy[k];
					x[k] += vx[k];
					t[k]++;
					int num = ((typeEff != 11) ? 4 : 3);
					if (t[k] > ((typeEff == 2) ? 4 : 2))
					{
						if (typeEff != 11)
							frame[k]++;
						t[k] = 0;
		...
		case 12:
		{
			for (int i = 0; i < sum; i++)
			{
				if (i % 3 != 0 && typeEff != 12 && TileMap.tileTypeAt(x[i], y[i] - GameCanvas.transY, 2))
					activeEff[i] = true;
				if (i % 3 == 0 && y[i] > GameCanvas.h + GameScr.cmy)
				{
					x[i] = Res.random(-10, GameCanvas.w + 300) + GameScr.cmx;
					y[i] = Res.random(-100, 0) + GameScr.cmy;
				}
				if (!activeEff[i])
				{
					y[i] += vy[i];
	...
	public void paintBehindTile(mGraphics g)
	{
		...
		case 8:
			...
			if (way != 1 && way != 2)
			{
				...
				g.drawRegion(imgFire2, 0, frameFire * 18, 8, 18, trans, xShip + num, yShip + 22, 3);
			}
			else
			{
