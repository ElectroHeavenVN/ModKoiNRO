...
public class GameScr : mScreen, IChatable
{
	...
	public override void paint(mGraphics g)
	{
		...
		for (int j = 0; j < Teleport.vTeleport.size(); j++)
		{
			((Teleport)Teleport.vTeleport.elementAt(j)).paintHole(g);
		}
		for (int k = 0; k < vNpc.size(); k++)
		{
			Npc npc = (Npc)vNpc.elementAt(k);
			if (npc.cHP > 0)
				npc.paintShadow(g);
		}
		for (int l = 0; l < vNpc.size(); l++)
		{
			((Npc)vNpc.elementAt(l)).paint(g);
		}
		g.translate(0, GameCanvas.transY);
		GameCanvas.debug("PA7", 1);
		GameCanvas.debug("PA8", 1);
		for (int m = 0; m < vCharInMap.size(); m++)
		{
			Char char2 = null;
			try
			{
				char2 = (Char)vCharInMap.elementAt(m);
			}
			catch (Exception ex)
			{
		...
		for (int n = 0; n < vMob.size(); n++)
		{
			((Mob)vMob.elementAt(n)).paint(g);
		}
		for (int num = 0; num < Teleport.vTeleport.size(); num++)
		{
			((Teleport)Teleport.vTeleport.elementAt(num)).paint(g);
		}
		...
		for (int num3 = 0; num3 < vCharInMap.size(); num3++)
		{
			...
			try
			{
				char4 = (Char)vCharInMap.elementAt(num3);
			}
			catch (Exception ex3)
			{
		...
		{
			((Npc)vNpc.elementAt(num5)).paintName(g);
		}
		...
		if (!isPaintOther)
		{
			...
			for (int num8 = 0; num8 < vCharInMap.size(); num8++)
			{
				...
				try
				{
					char6 = (Char)vCharInMap.elementAt(num8);
				}
				catch (Exception)
				{
				}
			...
			resetTranslate(g);
			int num9 = 100 + ((Char.vItemTime.size() != 0) ? (textTime.size() * 12) : 0);
			if (Char.myCharz().clan != null)
			{
				int num10 = 0;
				int num11 = 0;
				int num12 = (GameCanvas.h - 100 - 60) / 12;
				for (int num13 = 0; num13 < vCharInMap.size(); num13++)
				{
					Char char7 = (Char)vCharInMap.elementAt(num13);
					...
					if (char7.isOutX() && char7.cx < Char.myCharz().cx)
					{
						...
						if (num10 <= num14)
						{
							mFont.tahoma_7_green.drawString(g, char7.cName, 20, num9 - 12 + num10 * 12, mFont.LEFT, mFont.tahoma_7_grey);
							char7.paintHp(g, 10, num9 + num10 * 12 - 5);
							num10++;
						}
					}
					else if (char7.isOutX() && char7.cx > Char.myCharz().cx && num11 <= num12)
					{
						mFont.tahoma_7_green.drawString(g, char7.cName, GameCanvas.w - 25, num9 - 12 + num11 * 12, mFont.RIGHT, mFont.tahoma_7_grey);
						char7.paintHp(g, GameCanvas.w - 15, num9 + num11 * 12 - 5);
						num11++;
					}
				}
			}
			...
			for (int num15 = 0; num15 < Char.vItemTime.size(); num15++)
			{
				((ItemTime)Char.vItemTime.elementAt(num15)).paint(g, cmdMenu.x + 32 + num15 * 24, 55);
			}
			for (int num16 = 0; num16 < textTime.size(); num16++)
			{
				((ItemTime)textTime.elementAt(num16)).paintText(g, cmdMenu.x + ((Char.vItemTime.size() == 0) ? 25 : 5), ((Char.vItemTime.size() == 0) ? 45 : 90) + num16 * 12);
			}
		}
		int num17 = 0;
		...
		if (num18 > 200)
			num18 = 200;
		paintPhuBanBar(g, num17 + GameCanvas.w / 2, 0, num18);
