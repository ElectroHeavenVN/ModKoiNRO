...
public class AutoChat : IActionListener, IChatable
{
	...
	private static string stringAutoChatPublic;
	...
	private static string strAutoChatGlobal;

	private static int countChatGlobal;

	private static int availableGems;
	...
	private static string strAutoInbox;
			...
			if (ChatTextField.gI().strChat.Equals(inputDelayChatPublic[0]))
			{
				...
				try
				{
					...
					if (num > 0)
						try
						{
							stringAutoChatPublic = File.ReadAllText("Data\\TextChatPublic.ini");
			...
			else if (ChatTextField.gI().strChat.Equals(inputDelayChatGlobal[0]))
			{
				...
				try
				{
					...
					if (num2 > 0)
						try
						{
							strAutoChatGlobal = File.ReadAllText("Data\\TextChatGlobal.ini");
				...
				try
				{
					...
					if (num3 > 0)
						try
						{
							...
							availableGems = Char.myCharz().luong + Char.myCharz().luongKhoa;
	...
	public void perform(int idAction, object p)
	{
		...
		case 4:
			if (isAutoInbox)
			{
				...
				try
				{
					strAutoInbox = File.ReadAllText("Data\\TextChatInbox.ini");
					isAutoInbox = true;
					GameScr.info1.addInfo("Auto Inbox\n[STATUS: ON]", 0);
				}
				catch
	...
	public static void ShowMenu()
	{
		...
		myVector.addElement(new Command("Auto Inbox người trong khu\n" + (isAutoInbox ? "[STATUS: ON]" : "[STATUS: OFF]"), getInstance(), 4, null));
	...
	private static void ChatPublic()
	{
		...
		if (stringAutoChatPublic == null || stringAutoChatPublic.Equals(""))
			try
			{
				stringAutoChatPublic = File.ReadAllText("Data\\TextChatPublic.ini");
			}
	...
	private static void ChatGlobal()
	{
		...
		if (strAutoChatGlobal == null || strAutoChatGlobal.Equals(""))
			try
			{
				strAutoChatGlobal = File.ReadAllText("Data\\TextChatGlobal.ini");
			}
			...
			{
				strAutoChatGlobal = "Dũng đẹp trai!";
			}
		if (availableGems == num && mSystem.currentTimeMillis() - lastTimeChatGlobal > 1000L)
		...
		if (mSystem.currentTimeMillis() - lastTimeChatGlobal > delayAutoChatGlobal)
		{
			...
			countChatGlobal++;
			availableGems = num;
			Service.gI().chatGlobal(countChatGlobal + "dp: " + strAutoChatGlobal + " " + Res.random(100000, 999999));
		}
	}
	...
	private static void ChatInbox()
	{
		...
		for (int i = 0; i < GameScr.vCharInMap.size(); i++)
		{
			...
			if (isInboxable(@char) && @char.charID != Char.myCharz().charID && !listCharInbox.Contains(@char.charID))
				listCharInbox.Add(@char.charID);
		}
		if (strAutoInbox == null || strAutoInbox.Equals(""))
			try
			{
				strAutoInbox = File.ReadAllText("Data\\TextChatInbox.ini");
			}
			...
			{
				strAutoInbox = "Dũng đẹp trai!";
			}
		if (listCharInbox.Count > 0 && mSystem.currentTimeMillis() - lastTimeInbox > 2000)
		{
			lastTimeInbox = mSystem.currentTimeMillis();
			countChatInbox++;
			Service.gI().chatPlayer(countChatInbox + "dp: " + strAutoInbox + " " + Res.random(100000, 999999), listCharInbox[0]);
			listCharInbox.RemoveAt(0);
		}
	}

	private static bool isInboxable(Char gclass131_0)
	{
		if (gclass131_0 != null && gclass131_0.cName != null && !gclass131_0.cName.Equals("") && !char.IsUpper(char.Parse(gclass131_0.cName.Substring(0, 1))) && !gclass131_0.isPet && !gclass131_0.isMiniPet && !gclass131_0.cName.StartsWith("#"))
			return !gclass131_0.cName.StartsWith("$");
		return false;
	}
