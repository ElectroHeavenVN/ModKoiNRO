...
public class AutoSkill : IActionListener, IChatable
{
	...
	private static long[] lastTimeSentAttack;
	...
	public static void AutoSendAttack()
	{
		...
		if (mSystem.currentTimeMillis() - lastTimeSentAttack[mySkillIndex] > GetCoolDown(Char.myCharz().myskill))
		{
			if (GameScr.gI().isMeCanAttackMob(Char.myCharz().mobFocus))
			{
				Char.myCharz().myskill.lastTimeUseThisSkill = mSystem.currentTimeMillis();
				SendMobFocusAttack();
				lastTimeSentAttack[mySkillIndex] = mSystem.currentTimeMillis();
			}
	...
	private static void AutoSkillForPet()
	{
		...
		int mySkillIndex = GetMySkillIndex();
		if (mSystem.currentTimeMillis() - lastTimeSentAttack[mySkillIndex] > Char.myCharz().myskill.coolDown + 100 && GameScr.gI().isMeCanAttackMob(Char.myCharz().mobFocus))
		{
			SendMobFocusAttack();
			lastTimeSentAttack[mySkillIndex] = mSystem.currentTimeMillis();
	...
	public static bool isMeCanAttackChar(Char c)
	{
		if (TileMap.mapID == 113)
		{
			if (c != null && Char.myCharz().myskill != null)
			{
				if (c.cTypePk != 5)
					return c.cTypePk == 3;
				return true;
		...
		if (c != null && Char.myCharz().myskill != null)
		{
			if (c.statusMe != 14 && c.statusMe != 5 && Char.myCharz().myskill.template.type != 2 && ((Char.myCharz().cFlag == 8 && c.cFlag != 0) || (Char.myCharz().cFlag != 0 && c.cFlag == 8) || (Char.myCharz().cFlag != c.cFlag && Char.myCharz().cFlag != 0 && c.cFlag != 0) || (c.cTypePk == 3 && Char.myCharz().cTypePk == 3) || Char.myCharz().cTypePk == 5 || c.cTypePk == 5 || (Char.myCharz().cTypePk == 1 && c.cTypePk == 1) || (Char.myCharz().cTypePk == 4 && c.cTypePk == 4)))
				return true;
	...
	private static void SendCharFocusAttack()
	{
		try
		{
			MyVector myVector = new MyVector();
	...
	private static void SendMobFocusAttack()
	{
		try
		{
			MyVector myVector = new MyVector();
	...
	private static long GetCooldownAutoChangeFocus(Skill mySkill)
	{
		if (mySkill.coolDown <= 500)
			return 1000L;
		return (long)((double)mySkill.coolDown * 1.2 + 200.0);
	}
	...
	{
		isLoadKeySkill = true;
		lastTimeSentAttack = new long[8];
		isAutoUseSkills = new bool[8];
		lastTimeUseSkill = new long[8];
		timeAutoSkills = new long[8];
		listTargetAutoChangeFocus = new List<Char>();
		inputDelay = new string[2] { "Nhập delay", "mili giây" };
		smethod_0();
	}

	public static void FreezeMySelectedSkill()
	{
		int mySkillIndex = GetMySkillIndex();
