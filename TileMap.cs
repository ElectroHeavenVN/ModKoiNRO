...
public class TileMap
{
	...
	public static BgItem getBIById(int id)
	{
		...
		while (true)
		{
			if (num < vItemBg.size())
			{
				bgItem = (BgItem)vItemBg.elementAt(num);
				if (bgItem.id == id)
					break;
				num++;
				continue;
	...
	public static bool isOfflineMap()
	{
		...
		while (true)
		{
			if (num < offlineId.Length)
			{
				if (mapID == offlineId[num])
					break;
				num++;
				continue;
			}
	...
	public static bool isToOfflineMap()
	{
		...
		while (true)
		{
			if (num < toOfflineId.Length)
			{
				if (mapID == toOfflineId[num])
					break;
				num++;
				continue;
			}
	...
	public static void getTile()
	{
		if (Main.typeClient != 3 && Main.typeClient != 5)
		{
			...
			else if (GameCanvas.loadImageRMS("/t/" + tileID + "$1.png") != null)
			{
				...
				{
					imgTile[k] = GameCanvas.loadImageRMS("/t/" + tileID + "$" + (k + 1) + ".png");
				}
			}
			else
			{
		...
		else
		{
			...
			{
				imgTile[l] = GameCanvas.loadImage("/t/" + tileID + "/" + (l + 1) + ".png");
			}
		}
	}
		...
		while (true)
		{
			if (num < GameScr.gssxe)
			{
				for (int j = GameScr.gssy; j < GameScr.gssye; j++)
				{
					...
					if ((tileTypeAt(num, j) & 0x100) == 256)
						continue;
					if ((tileTypeAt(num, j) & 0x20) == 32)
					{
						g.drawRegion(imgWaterfall, 0, 24 * (GameCanvas.gameTick % 8 >> 1), 24, 24, 0, num * size, j * size, 0);
						continue;
					}
					if ((tileTypeAt(num, j) & 0x80) == 128)
					{
						g.drawRegion(imgTopWaterfall, 0, 24 * (GameCanvas.gameTick % 8 >> 1), 24, 24, 0, num * size, j * size, 0);
						continue;
					}
					if (tileID == 13)
					{
					...
					if (tileID == 2 && (tileTypeAt(num, j) & 0x200) == 512 && num2 != -1)
					{
						paintTile(g, num2, num * size, j * size, 24, 1);
						paintTile(g, num2, num * size, j * size + 1, 24, 24);
					}
					if ((tileTypeAt(num, j) & 0x10) == 16)
					{
						bx = num * size - GameScr.cmx;
						dbx = bx - GameScr.gW2;
						dfx = (size - 2) * dbx / size;
						fx = dfx + GameScr.gW2;
						paintTile(g, num2, fx + GameScr.cmx, j * size, 24, 24);
					}
					else if ((tileTypeAt(num, j) & 0x200) == 512)
					{
						if (num2 != -1)
						{
							paintTile(g, num2, num * size, j * size, 24, 1);
