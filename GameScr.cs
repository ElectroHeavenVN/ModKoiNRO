...
public class GameScr : mScreen, IChatable
{
	...
	{
		int num = Char.myCharz().arrItemBag.Length - 1;
		while (true)
		{
			if (num >= 0)
			{
	...
	public void readPart()
	{
		...
		try
		{
			...
			for (int i = 0; i < num; i++)
			{
				sbyte type = dataInputStream.readByte();
				...
				if (Char.myCharz().charFocus != null)
				{
					...
					if (!Char.myCharz().isMeCanAttackOtherPlayer(Char.myCharz().charFocus) && !Char.myCharz().isSelectingSkillBuffToPlayer())
					{
						...
						else
						{
							...
							Char.myCharz().currentMovePoint = new MovePoint(Char.myCharz().charFocus.cx + num12 * num13, Char.myCharz().charFocus.cy);
							Char.myCharz().endMovePointCommand = new Command(null, null, 8002, null);
							GameCanvas.clearKeyHold();
							GameCanvas.clearKeyPressed();
						}
					...
					int num16 = (Char.myCharz().myskill.dx - 20) * ((Char.myCharz().cx > Char.myCharz().charFocus.cx) ? 1 : (-1));
	...
	private bool checkClipTopChatPopUp(int xClick, int yClick)
	{
		if (Equals(info2) && gI().popUpYesNo != null)
			return false;
		if (info2.info.info != null && info2.info.info.charInfo != null && inRectangle(x: Res.abs(info2.cmx) + info2.info.X - 40, y: Res.abs(info2.cmy) + info2.info.Y, xClick: xClick - cmx, yClick: yClick - cmy, w: 200, h: info2.info.H))
		{
			info2.doClick(10);
			return true;
		}
	...
	private bool checkClickToPopup(int xClick, int yClick)
	{
		...
		while (true)
		{
			if (num < PopUp.vPopups.size())
			{
				popUp = (PopUp)PopUp.vPopups.elementAt(num);
	...
	private void checkAuto()
	{
		...
		if (GameCanvas.keyPressed[(!Main.isPC) ? 5 : 25] && !isPaintPopup())
		{
			if (auto == 0)
			{
				if (num - lastFire < 800L && checkSkillValid2() && (Char.myCharz().mobFocus != null || (Char.myCharz().charFocus != null && Char.myCharz().isMeCanAttackOtherPlayer(Char.myCharz().charFocus))))
				{
					Res.outz("toi day");
					auto = 10;
					GameCanvas.keyPressed[(!Main.isPC) ? 5 : 25] = false;
	...
	public bool checkClickToCapcha()
	{
		if (mobCapcha == null)
			return false;
		return GameCanvas.isPointerHoldIn((GameCanvas.w - 5 * disXC) / 2, w: 5 * disXC, y: GameCanvas.h - 40, h: disXC);
	}
	...
	private void updateKeyTouchControl()
	{
		...
		else if (isHaveSelectSkill)
		{
			...
			{
				if (MainMod.isSanBoss)
				{
					for (int i = 0; i < MainMod.listBosses.Count; i++)
					{
						if (!GameCanvas.isPointerHoldIn(GameCanvas.w - 23, 42 + 10 * i, 23, 9))
							continue;
						GameCanvas.isPointerJustDown = false;
						isPointerDowning = false;
						if (!GameCanvas.isPointerClick || !GameCanvas.isPointerJustRelease)
						...
						{
							for (int j = 0; j < TileMap.mapNames.Length; j++)
							{
								if (TileMap.mapNames[j] == MainMod.listBosses[i].MapName)
								{
									AutoMap.Xmap(j);
									break;
								}
				...
				{
					for (int k = 0; k < MainMod.listCharsInMap.Count; k++)
					{
						if (!GameCanvas.isPointerHoldIn(GameCanvas.w - MainMod.widthRect, 95 + (MainMod.heightRect + 1) * k, MainMod.widthRect, MainMod.heightRect))
							continue;
						GameCanvas.isPointerJustDown = false;
						isPointerDowning = false;
						if (GameCanvas.isPointerClick && GameCanvas.isPointerJustRelease)
						{
							Char @char = MainMod.listCharsInMap[k];
							if (Char.myCharz().charFocus != null && Char.myCharz().charFocus.cName == @char.cName)
								MainMod.TeleportTo(Char.myCharz().charFocus.cx, Char.myCharz().charFocus.cy);
							else
								MainMod.FocusTo(@char.charID);
							Char.myCharz().currentMovePoint = null;
							GameCanvas.clearAllPointerEvent();
							return;
						}
			...
			if (MainMod.isShowCharsInMap && MainMod.isMeInNRDMap())
			{
				for (int l = 0; l < MainMod.listCharsInMap.Count; l++)
				{
					if (!GameCanvas.isPointerHoldIn(GameCanvas.w - MainMod.widthRect, 35 + (MainMod.heightRect + 1) * l, MainMod.widthRect, MainMod.heightRect))
						continue;
					GameCanvas.isPointerJustDown = false;
					isPointerDowning = false;
					if (GameCanvas.isPointerClick && GameCanvas.isPointerJustRelease)
					{
						Char char2 = MainMod.listCharsInMap[l];
						if (Char.myCharz().charFocus != null && Char.myCharz().charFocus.cName == char2.cName)
							MainMod.TeleportTo(Char.myCharz().charFocus.cx, Char.myCharz().charFocus.cy);
						else
							MainMod.FocusTo(char2.charID);
						Char.myCharz().currentMovePoint = null;
						GameCanvas.clearAllPointerEvent();
						return;
					}
	...
	public void paintEffect(mGraphics g)
	{
		...
		{
			((Effect2)Effect2.vEffect2Outside.elementAt(j)).paint(g);
		}
	}
	...
	public override void paint(mGraphics g)
	{
		...
		for (int k = 0; k < vNpc.size(); k++)
		{
			((Npc)vNpc.elementAt(k)).paint(g);
		}
		g.translate(0, GameCanvas.transY);
		GameCanvas.debug("PA7", 1);
		GameCanvas.debug("PA8", 1);
		for (int l = 0; l < vCharInMap.size(); l++)
		{
			Char @char = null;
			try
			{
				@char = (Char)vCharInMap.elementAt(l);
			}
			catch (Exception ex)
			{
		...
		{
			((Mob)vMob.elementAt(m)).paint(g);
		}
		for (int n = 0; n < Teleport.vTeleport.size(); n++)
		{
			((Teleport)Teleport.vTeleport.elementAt(n)).paint(g);
		}
		for (int num = 0; num < vCharInMap.size(); num++)
		{
			...
			try
			{
				char2 = (Char)vCharInMap.elementAt(num);
			}
			catch (Exception)
			{
			}
			if (char2 != null && (!GameCanvas.panel.isShow || !GameCanvas.panel.isTypeShop()))
				char2.paint(g);
		}
		...
		for (int num2 = 0; num2 < vCharInMap.size(); num2++)
		{
			Char char3 = null;
			try
			{
				char3 = (Char)vCharInMap.elementAt(num2);
			}
			catch (Exception ex3)
			{
				Cout.LogError("Loi ham paint char gamescr: " + ex3.ToString());
			}
			if (char3 != null && (!GameCanvas.panel.isShow || !GameCanvas.panel.isTypeShop()) && char3.skillPaint != null && char3.skillInfoPaint() != null && char3.indexSkill < char3.skillInfoPaint().Length)
			{
				char3.paintCharWithSkill(g);
				char3.paintMount2(g);
			}
		}
		for (int num3 = 0; num3 < vItemMap.size(); num3++)
		{
			((ItemMap)vItemMap.elementAt(num3)).paint(g);
		}
		g.translate(0, -GameCanvas.transY);
		...
		paintEffect(g);
		for (int num4 = 0; num4 < vNpc.size(); num4++)
		{
			((Npc)vNpc.elementAt(num4)).paintName(g);
		}
		EffecMn.paintLayer3(g);
		for (int num5 = 0; num5 < vNpc.size(); num5++)
		{
			Npc npc2 = (Npc)vNpc.elementAt(num5);
			if (npc2.chatInfo != null)
				npc2?.chatInfo.paint(g, npc2.cx, npc2.cy - npc2.ch - GameCanvas.transY, npc2.cdir);
		}
		for (int num6 = 0; num6 < vCharInMap.size(); num6++)
		{
			Char char4 = null;
			try
			{
				char4 = (Char)vCharInMap.elementAt(num6);
			}
			catch (Exception)
			{
			}
			if (char4 != null && char4.chatInfo != null)
				char4.chatInfo.paint(g, char4.cx, char4.cy - char4.ch, char4.cdir);
		}
		...
		{
			g.setColor(0);
			g.fillRect(3, 88, 54, 8);
			g.setColor(16711680);
			g.setClip(5, 90, mabuPercent, 4);
			g.fillRect(5, 90, 50, 4);
			g.setClip(0, 0, 3000, 3000);
			mFont.tahoma_7b_white.drawString(g, "Mabu", 30, 98, 2, mFont.tahoma_7b_dark);
		}
		...
		for (int num7 = 0; num7 < vCharInMap.size(); num7++)
		{
			Char char5 = null;
			try
			{
				char5 = (Char)vCharInMap.elementAt(num7);
			}
			catch (Exception)
			{
			}
			if (char5 != null && char5.isFusion && Char.isCharInScreen(char5))
			{
				char5.tFusion++;
				if (GameCanvas.gameTick % 3 == 0)
				{
				...
				if (char5.tFusion >= 100)
					char5.fusionComplete();
			}
		}
		...
		if (Char.myCharz().clan != null)
		{
			int num9 = 0;
			int num10 = 0;
			int num11 = (GameCanvas.h - 100 - 60) / 12;
			for (int num12 = 0; num12 < vCharInMap.size(); num12++)
			{
				Char char6 = (Char)vCharInMap.elementAt(num12);
				if (char6.clanID == -1 || char6.clanID != Char.myCharz().clan.ID)
					continue;
				if (char6.isOutX() && char6.cx < Char.myCharz().cx)
				{
					int num13 = num11;
					...
					if (num9 <= num13)
					{
						mFont.tahoma_7_green.drawString(g, char6.cName, 20, num8 - 12 + num9 * 12, mFont.LEFT, mFont.tahoma_7_grey);
						char6.paintHp(g, 10, num8 + num9 * 12 - 5);
						num9++;
					}
				}
				else if (char6.isOutX() && char6.cx > Char.myCharz().cx && num10 <= num11)
				{
					mFont.tahoma_7_green.drawString(g, char6.cName, GameCanvas.w - 25, num8 - 12 + num10 * 12, mFont.RIGHT, mFont.tahoma_7_grey);
					char6.paintHp(g, GameCanvas.w - 15, num8 + num10 * 12 - 5);
					num10++;
				}
			}
		}
		...
		for (int num14 = 0; num14 < Char.vItemTime.size(); num14++)
		{
			((ItemTime)Char.vItemTime.elementAt(num14)).paint(g, cmdMenu.x + 32 + num14 * 24, 55);
		}
		for (int num15 = 0; num15 < textTime.size(); num15++)
		{
			((ItemTime)textTime.elementAt(num15)).paintText(g, cmdMenu.x + ((Char.vItemTime.size() == 0) ? 25 : 5), ((Char.vItemTime.size() == 0) ? 45 : 90) + num15 * 12);
		}
	}
	...
	private void paintWaypointArrow(mGraphics g)
	{
		...
		for (int i = 0; i < TileMap.vGo.size(); i++)
		{
			...
			{
				if (waypoint.minX >= 0 && waypoint.minX < 24)
				{
					if (!GameCanvas.isTouch)
			...
			else if (waypoint.maxY <= TileMap.pxh / 2)
			{
			...
			if (Char.myCharz().isInEnterOfflinePoint() == null && Char.myCharz().isInEnterOnlinePoint() == null)
			{
				if (Char.myCharz().mobFocus != null)
				{
					if (Char.myCharz().mobFocus.getTemplate() != null)
						mFont.tahoma_7b_white.drawString(g, Char.myCharz().mobFocus.getTemplate().name, imgScrW / 2, 9 + mGraphics.addYWhenOpenKeyBoard, mFont.CENTER);
					if (Char.myCharz().mobFocus.templateId != -1)
						mFont.tahoma_7_red.drawString(g, NinjaUtil.getMoneys(Char.myCharz().mobFocus.hp) + string.Empty, imgScrW / 2, 22 + mGraphics.addYWhenOpenKeyBoard, mFont.CENTER);
				}
				else if (Char.myCharz().npcFocus != null)
				{
					mFont.tahoma_7b_white.drawString(g, Char.myCharz().npcFocus.template.name, imgScrW / 2, 9 + mGraphics.addYWhenOpenKeyBoard, mFont.CENTER);
					if (Char.myCharz().npcFocus.template.npcTemplateId == 4)
						mFont.tahoma_7_white.drawString(g, gI().magicTree.currPeas + "/" + gI().magicTree.maxPeas, imgScrW / 2, 22 + mGraphics.addYWhenOpenKeyBoard, mFont.CENTER);
				}
				else if (Char.myCharz().charFocus != null)
				{
					mFont.tahoma_7b_white.drawString(g, Char.myCharz().charFocus.cName, imgScrW / 2, 9 + mGraphics.addYWhenOpenKeyBoard, mFont.CENTER);
					mFont.tahoma_7_red.drawString(g, NinjaUtil.getMoneys(Char.myCharz().charFocus.cHP) + string.Empty, imgScrW / 2, 22 + mGraphics.addYWhenOpenKeyBoard, mFont.CENTER);
				}
				else if (Char.myCharz().itemFocus != null)
				{
					mFont.tahoma_7b_white.drawString(g, Char.myCharz().itemFocus.template.name, imgScrW / 2, 9 + mGraphics.addYWhenOpenKeyBoard, mFont.CENTER);
				}
				else
				{
					mFont.tahoma_7b_white.drawString(g, Char.myCharz().cName, imgScrW / 2, 9 + mGraphics.addYWhenOpenKeyBoard, mFont.CENTER);
					mFont.tahoma_7_yellow.drawString(g, NinjaUtil.getMoneys(Char.myCharz().cPower) + string.Empty, imgScrW / 2, 22 + mGraphics.addYWhenOpenKeyBoard, mFont.CENTER);
				}
			}
			else
			...
			else if (Char.myCharz().statusMe != 14)
			{
				if (gamePad.isSmallGamePad)
				{
					if (isAnalog != 1)
					{
						...
						g.drawImage((mScreen.keyTouch != 10) ? imgHP1 : imgHP2, xHP + 4, yHP + 4, 0);
	...
	{
		if (GameCanvas.panel.isShowZone)
			InfoDlg.hide();
		try
		{
	...
	public void actionPerform(int idAction, object p)
	{
		...
		{
			Char obj3 = (Char)p;
			Service.gI().friend(1, obj3.charID);
			break;
		}
		case 11113:
		{
	...
	private void paintGamePad(mGraphics g)
	{
		if (isAnalog != 0 && Char.myCharz().statusMe != 14)
		{
			g.drawImage((mScreen.keyTouch == 5 || mScreen.keyMouse == 5) ? imgFire1 : imgFire0, xF + 20, yF + 20, mGraphics.HCENTER | mGraphics.VCENTER);
			gamePad.paint(g);
