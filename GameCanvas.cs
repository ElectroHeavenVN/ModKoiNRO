...
public class GameCanvas : IActionListener
{
	...
	public static Image[] imgSunSpec;
	...
	public void update()
	{
		...
		try
		{
			...
			if (gameTick > 10000)
			{
				if (mSystem.currentTimeMillis() - lastTimePress > 20000L && currentScreen == loginScr)
	...
	{
		if (mSystem.currentTimeMillis() - lastTimePress >= 800L)
			return true;
		return false;
	}
	...
	public void paintChangeMap(mGraphics g)
	{
		...
		g.setColor(0);
		g.fillRect(0, 0, w, h);
		g.drawImage(MainMod.logoServerListScreen, w / 2, h / 2, StaticObj.BOTTOM_HCENTER);
	...
	public void perform(int idAction, object p)
	{
		...
		case 88837:
		{
			...
			try
			{
				Service.gI().openLockAccProtect(int.Parse(text3.Trim()));
				break;
			}
			catch (Exception ex4)
			{
